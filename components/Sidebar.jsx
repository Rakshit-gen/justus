    ...
    <aside className="flex h-full w-full flex-col border-r border-slate-200 bg-white md:w-80">
      ...
      <div className="flex items-center justify-between gap-2 border-b border-slate-200 px-3 py-3 sm:px-4">
        <button
          ...
          className="flex min-w-0 flex-1 items-center gap-3 rounded-lg p-1 -m-1 text-left transition hover:bg-slate-50"
        >
          ...
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className={clsx('text-[11px]', connected ? 'text-emerald-600' : 'text-amber-600')}>
                {connected ? 'online' : 'reconnecting…'}
              </span>
            </div>
          </div>
        </button>
        <button onClick={onOpenAddFriend} className="rounded-md p-2 text-slate-500 transition hover:bg-slate-100 hover:text-slate-700" aria-label="Add friend" title="Add friend">
        ...
        <button onClick={onOpenSettings} className="rounded-md p-2 text-slate-500 transition hover:bg-slate-100 hover:text-slate-700" aria-label="Settings">
          <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className="px-3 py-3 sm:px-4">
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-lg border border-slate-200 bg-slate-50 py-2 pl-9 pr-3 text-sm outline-none transition placeholder:text-slate-400 focus:border-brand-500 focus:bg-white focus:ring-4 focus:ring-brand-100"
          />
        </div>
      </div>
      ...
      {incomingCount > 0 && (
        <button
          ...
          className="mx-3 mb-2 flex items-center justify-between gap-2 rounded-xl bg-gradient-to-br from-amber-50 to-orange-50 px-3 py-2.5 text-left ring-1 ring-amber-200 transition hover:from-amber-100 hover:to-orange-100 sm:mx-4"
        >
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              <div className="text-xs font-semibold text-amber-900">Friend requests</div>
              <div className="text-[11px] text-amber-700">{incomingCount} pending</div>
            </div>
          </div>
          <svg viewBox="0 0 24 24" className="h-4 w-4 shrink-0 text-amber-600" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round">
...
function SidebarRow({ row, active, onSelect, onLongPress }) {
  ...
  return (
    <div
      ...
      tabIndex={0}
      className={clsx(
        'flex w-full cursor-pointer items-center gap-3 px-3 py-3 text-left transition sm:px-4',
        active ? 'bg-gradient-to-r from-brand-50 to-brand-100/40' : 'hover:bg-slate-50'
      )}
    >
      <UserAvatar name={u.name} color={u.avatarColor} avatarUrl={u.avatarUrl} size={44} online={u.isOnline} showDot />
      <div className="min-w-0 flex-1">
        <div className="flex items-center justify-between gap-2">
          <div className={clsx('flex min-w-0 items-center gap-1.5', active ? 'text-brand-700' : 'text-slate-900')}>
            <span className="truncate text-sm font-medium">{u.name}</span>
            {row.pinned && (
              <svg viewBox="0 0 24 24" className="h-3 w-3 shrink-0 text-slate-400" fill="currentColor" stroke="none" aria-label="Pinned">
                <path d="M16 12V4h1a1 1 0 0 0 0-2H7a1 1 0 0 0 0 2h1v8l-3 3v2h6v6l1 1 1-1v-6h6v-2l-3-3z" />
              </svg>
            )}
          </div>
          {row.updatedAt && (
            <span className={clsx('shrink-0 text-[11px]', row.unread > 0 ? 'text-brand-600 font-medium' : 'text-slate-400')}>
              {formatSidebarTime(row.updatedAt)}
            </span>
          )}
        </div>
        <div className="mt-0.5 flex items-center justify-between gap-2">
          <div className={clsx('truncate text-xs', row.unread > 0 ? 'text-slate-700 font-medium' : 'text-slate-500')}>
            {row.lastMessage?.content || u.bio || (u.isOnline ? 'online' : 'tap to start chatting')}
          </div>
          {row.unread > 0 && (
            <span className="ml-2 inline-flex h-5 min-w-[20px] items-center justify-center rounded-full bg-gradient-to-br from-brand-500 to-brand-700 px-1.5 text-[11px] font-semibold text-white shadow-sm shadow-brand-600/30">
...
function EmptySidebar({ query, hasFriends, onAdd }) {
  if (query) {
    return <div className="px-4 py-10 text-center text-sm text-slate-400">No matches for “{query}”</div>;
  }
  return (
