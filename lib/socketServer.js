...
async function deliverMessage(io, { senderId, recipientId, content, attachments, replyTo, excludeSocketId }) {
  ...
  // Disappearing messages — if this conversation has a TTL set, stamp
  // an expiresAt on the new message so Mongo's TTL monitor removes it.
  const ttlSeconds = Number(convo.disappearingTtl || 0);
...
function initSocket(io) {
  ...
  io.on('connection', async (socket) => {
    ...
    // Either participant may toggle disappearing messages for this convo.
    // ttlSeconds: 0 (off), or a positive number — server clamps to a safe set.
    socket.on('convo:disappearing:set', async ({ otherUserId, ttlSeconds }, ack) => {
      try {
        if (!otherUserId) return ack?.({ error: 'No recipient' });
        const allowed = new Set([0, 86400, 604800, 2592000]); // off, 24h, 7d, 30d
        const ttl = Number(ttlSeconds) || 0;
        if (!allowed.has(ttl)) return ack?.({ error: 'Invalid TTL' });
        await connectDB();
        ...
        // Both participants need to see the new banner immediately.
        emitToUser(io, userId, 'convo:disappearing:updated', payload);
        emitToUser(io, otherUserId, 'convo:disappearing:updated', {
          ...payload,
          otherUserId: userId,
        });
        ack?.({ ok: true, ttlSeconds: ttl });
      } catch (err) {
        console.error('[convo:disappearing:set]', err);
        ack?.({ error: 'Failed' });
      }
    });
